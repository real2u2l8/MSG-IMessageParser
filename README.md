...
- **Hex 변환**: 모든 스트림 데이터를 16진수 형태로 변환하여 저장
...
```
output_directory/
├── __nameid_version/          # Named Property 매핑 정보
├── __recip_version/           # 수신자 테이블 정보
├── __properties_version/      # 메일 속성 정보
├── __attach_version/          # 첨부파일 정보
├── __substg1.0_0C1F.hex      # 발신자 이메일 주소
├── __substg1.0_0E04.hex      # 수신자(To) 정보
├── __substg1.0_0E03.hex      # 참조(Cc) 정보
├── __substg1.0_0E02.hex      # 숨은참조(Bcc) 정보
├── __substg1.0_0037.hex      # 메일 제목
├── __substg1.0_1000.hex      # 메일 본문
├── __substg1.0_007D.hex      # 제목 접두사 (RE:, FW: 등)
└── ... (기타 모든 스트림)
```
